...
public abstract class AILayerCapuchinItem extends RenderLayer<EntityCapuchinMonkey, ModelCapuchinMonkey> {


    public AILayerCapuchinItem(RenderLayerParent<EntityCapuchinMonkey, ModelCapuchinMonkey> pRenderer) {
        super(pRenderer);
    }
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityCapuchinMonkey entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn.hasDart()) {
            ...
            ItemStack itemstack;
            if (AInteractionConfig.capuchinbomb && AInteractionConfig.goofymode) itemstack = new ItemStack(Items.TNT);
