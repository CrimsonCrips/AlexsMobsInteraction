...
public class AISeagullAIStealFromPlayers extends Goal {
    ...
    public AISeagullAIStealFromPlayers(EntitySeagull entitySeagull) {
    ...
    public boolean canUse() {
        ...
        if(this.seagull.getMainHandItem().isEmpty()){
            ...
            if (AInteractionConfig.seagullsombrero) {
                if (valid != null && !valid.getItemBySlot(EquipmentSlot.HEAD).is(AMItemRegistry.SOMBRERO.get())) {
                    target = valid;
                    return true;
                }
            } else {
    ...
    public void tick(){
        ...
        if (AInteractionConfig.seagullbuff){
            if (seagull.getMainHandItem().is(Items.ENCHANTED_GOLDEN_APPLE)) {
                seagull.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 1000, 1));
                seagull.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 3000, 2));
            }
        }
        if(fleeTime > 0){
            if(fleeVec == null){
