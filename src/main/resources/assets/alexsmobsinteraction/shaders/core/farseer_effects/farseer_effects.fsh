...
void main() {
    ...
    float squiggles = angleAroundCenter * 50.0 + sin(GameTime * 2000.0 + distFromCenter * 25.0) * 2.0;
    float rayFromAngle = sin(squiggles - GameTime * 20.0);

    float radius = distFromCenter * 10.0 + GameTime * 100.0 + angleAroundCenter * INV_PI;
    float degrees = angleAroundCenter * 0.0625 + squiggles * 0.0625 + GameTime * 150.0;
    vec2 polarCoords = vec2(radius, degrees);
